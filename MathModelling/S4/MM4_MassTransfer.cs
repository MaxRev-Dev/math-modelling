...
namespace MM.S4
{
    class MM4_MassTransfer : BaseMethod
    {
        [ReflectedUICoefs]
        public int
            t_ = 5,
        ...
        double[][] MassTransfer(double NN, double D, double[][] U, double y, double Cm, double l, int n, int t)
        {
            var h = l * 1.0 / n;
            int T = 1;
            ...
            for (int i = 1; i < t + 1; i++)
            {
                ...
                for (int m = 1; m < n; m++)
                {
                    ...
                    B[m] = (a[m] * B[m - 1] + tIrs[i - 1][m] + f[m]) / (c[m] - a[m] * L[m - 1]);
                }

                inline[n] = (-U[0][n] * 10 * h / D + B[n - 1]) / (1 - U[0][n] * h / D - L[n - 1]);
                for (int k = n - 1; k > 0; k--)
                {
