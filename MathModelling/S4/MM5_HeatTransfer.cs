...
namespace MM.S4
{
    class MM5_HeatTransfer : BaseMethod
    {
        [ReflectedUICoefs]
        public int
            t_ = 5,
        ...
        double[][] HeatTransfer(double Ct, double lam, double[][] U, double Cp, double Tz, double alfa, int l, int n, int t)
        {
            var h = l * 1.0 / n;
            var c0 = 5;
            int T = 1;
            ...
            for (int i = 1; i < t + 1; i++)
            {
                ...
                for (int j = 0; j < n + 1; j++)
                {
                    var N = lam / (1 + h * Math.Abs(1003 * Cp * U[i][j]) / (2 * lam));
                ...
                for (int m = 1; m < n; m++)
                {
                    ...
                    B[m] = (a[m] * B[m - 1] + tIrs[i - 1][m]) / (c[m] - a[m] * L[m - 1]);
                }

                inline[n] = (-alfa * h * L[1] * Tz + lam + B[1]) / (-(lam + alfa * h) * L[1] + lam);
                for (int k = n - 1; k > 0; k--)
                {
