...
namespace MM.S5
{
    internal class S5M4_2DMassHeatTrasferUndergr : BaseMethod
    {
        ...
        public double[][][] CalculateMassTransfer2D()
        {
            ...
            var bx_const = M / h_2;
            var cx_const = ax_const + bx_const + gamma / (2 * D) + sigma / (D * tau);
            ...
            void FillOnZero()
            {
                var layer = new double[Ny + 1].Select(x => new double[Nx + 1]).ToArray();
                for (int v = 0; v < Ny + 1; v++)
                {
                    layer[v][0] = C1;
                    for (int i = 1; i < Nx; i++)
                        layer[v][i] = v == 0 ? Cm : (C2 - C1) * hx * i / Lx + C1;
                    layer[v][Nx] = C2;
                }
            ...
            void FillByHalf(int layerTimek)
            {
                ...
                for (int v = 0; v < Ny + 1; v++)
                {
                    ...
                    else
                    {
                        ...
                        for (var i = 1; i < Nx; i++)
                        {
                            alfa[i] = bx_const / (cx_const - alfa[i - 1] * ax_const);

                            var F1 = Dt * (hC[v][i - 1] - 2 * hC[v][i] + hC[v][i + 1]) / (D * h_2);
            ...
            void FillByFull(int layerTimek)
            {
                ...
                for (int v = 0; v < Nx + 1; v++)
                {
                    ...
                    else
                    {
                        ...
                        for (var i = 1; i < Ny; i++)
                        {
                            alfa[i] = by_const / (cy_const - alfa[i - 1] * ay_const);

                            var F1 = Dt * (hC[i - 1][v] - 2 * hC[i][v] + hC[i + 1][v]) / (D * h_2);
            ...
            for (int layerTimeK = 0; layerTimeK < times * 2; layerTimeK++) // time layers
            {
                if (layerTimeK == 0)
                {
                    FillOnZero();
        ...
        public double[][][] CalculateHeatTransfer2D()
        {
            ...
            for (int layerTimeI = 0; layerTimeI < times * 2; layerTimeI++) // time layers
            {
                if (layerTimeI == 0)
                {
                    FillOnZero();
